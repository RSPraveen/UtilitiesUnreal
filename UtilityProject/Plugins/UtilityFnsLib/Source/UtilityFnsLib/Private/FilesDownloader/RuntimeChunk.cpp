...
FRuntimeChunk::FRuntimeChunk()
	: bCanceled(false)
{}

FRuntimeChunk::~FRuntimeChunk()
{
	UE_LOG(LogTemp, Warning, TEXT("FRuntimeChunk destroyed"));
}

TFuture<FRuntimeChunkResult> FRuntimeChunk::DownloadFile(const FString& URL, float Timeout, const FString& ContentType, int64 MaxChunkSize, const TFunction<void(int64, int64)>& OnProgress)
{
	if (bCanceled)
	{
		UE_LOG(LogTemp, Warning, TEXT("Canceled file download from %s"), *URL);
		return MakeFulfilledPromise<FRuntimeChunkResult>(FRuntimeChunkResult{ EDownloadToMemoryResult::Cancelled, TArray64<uint8>() }).GetFuture();
	}

	TSharedPtr<TPromise<FRuntimeChunkResult>> PromisePtr = MakeShared<TPromise<FRuntimeChunkResult>>();
	TWeakPtr<FRuntimeChunk> WeakThisPtr = AsShared();
	GetContentSize(URL, Timeout).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType, MaxChunkSize, OnProgress](int64 ContentSize) mutable
		{
			TSharedPtr<FRuntimeChunk> SharedThis = WeakThisPtr.Pin();
			if (!SharedThis.IsValid())
			{
				UE_LOG(LogTemp, Warning, TEXT("Failed to download file chunk from %s:  has been destroyed"), *URL);
				PromisePtr->SetValue(FRuntimeChunkResult{ EDownloadToMemoryResult::DownloadFailed, TArray64<uint8>() });
				return;
			}
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("Canceled file download from %s"), *URL);
				PromisePtr->SetValue(FRuntimeChunkResult{ EDownloadToMemoryResult::Cancelled, TArray64<uint8>() });
				return;
			}

			auto DownloadByPayload = [SharedThis, WeakThisPtr, PromisePtr, URL, Timeout, ContentType, OnProgress]()
			{
				SharedThis->DownloadFileByPayload(URL, Timeout, ContentType, OnProgress).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType, OnProgress](FRuntimeChunkResult Result) mutable
					{
						TSharedPtr<FRuntimeChunk> SharedThis = WeakThisPtr.Pin();
						if (!SharedThis.IsValid())
						{
						...
						PromisePtr->SetValue(FRuntimeChunkResult{ Result.Result, MoveTemp(Result.Data) });
					});
			};
			...
			auto OnChunkDownloaded = [WeakThisPtr, PromisePtr, URL, ContentSize, Timeout, ContentType, OnProgress, DownloadByPayload, OverallDownloadedDataPtr, bChunkDownloadedFilledPtr, ChunkOffsetPtr, OnChunkDownloadedFilled](TArray64<uint8>&& ResultData) mutable
			{
				...
				{
					PromisePtr->SetValue(FRuntimeChunkResult{ EDownloadToMemoryResult::Success, MoveTemp(*OverallDownloadedDataPtr.Get()) });
					OnChunkDownloadedFilled();
					return;
				}
			...
			SharedThis->DownloadFilePerChunk(URL, Timeout, ContentType, MaxChunkSize, ChunkRange, OnProgress, OnChunkDownloaded).Next([PromisePtr, bChunkDownloadedFilledPtr, URL, OverallDownloadedDataPtr, OnChunkDownloadedFilled, DownloadByPayload](EDownloadToMemoryResult Result) mutable
				{
					...
					if (bChunkDownloadedFilledPtr.IsValid() && (*bChunkDownloadedFilledPtr.Get() == false))
					{
						...
						PromisePtr->SetValue(FRuntimeChunkResult{ Result, MoveTemp(*OverallDownloadedDataPtr.Get()) });
					}
				});
		});
...
TFuture<EDownloadToMemoryResult> FRuntimeChunk::DownloadFilePerChunk(const FString& URL, float Timeout, const FString& ContentType, int64 MaxChunkSize, FInt64Vector2 ChunkRange, const TFunction<void(int64, int64)>& OnProgress, const TFunction<void(TArray64<uint8>&&)>& OnChunkDownloaded)
{
	if (bCanceled)
	{
		UE_LOG(LogTemp, Warning, TEXT("Canceled file chunk download from %s"), *URL);
	...
	GetContentSize(URL, Timeout).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType, MaxChunkSize, OnProgress, OnChunkDownloaded, ChunkRange](int64 ContentSize) mutable
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("Unable to get content size for %s. Trying to download the file by payload"), *URL);
				SharedThis->DownloadFileByPayload(URL, Timeout, ContentType, OnProgress).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType, OnChunkDownloaded, OnProgress](FRuntimeChunkResult Result) mutable
					{
						TSharedPtr<FRuntimeChunk> SharedThis = WeakThisPtr.Pin();
						if (!SharedThis.IsValid())
						{
			...
			SharedThis->DownloadFileByChunk(URL, Timeout, ContentType, ContentSize, ChunkRange, OnProgressInternal).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType, ContentSize, MaxChunkSize, OnChunkDownloaded, OnProgress, ChunkRange](FRuntimeChunkResult&& Result)
				{
					TSharedPtr<FRuntimeChunk> SharedThis = WeakThisPtr.Pin();
					if (!SharedThis.IsValid())
					{
...
TFuture<FRuntimeChunkResult> FRuntimeChunk::DownloadFileByChunk(const FString& URL, float Timeout, const FString& ContentType, int64 ContentSize, FInt64Vector2 ChunkRange, const TFunction<void(int64, int64)>& OnProgress)
{
	if (bCanceled)
	{
		UE_LOG(LogTemp, Warning, TEXT("Canceled file download from %s"), *URL);
	...
	if (ChunkRange.X < 0 || ChunkRange.Y <= 0 || ChunkRange.X > ChunkRange.Y)
	{
		...
		return MakeFulfilledPromise<FRuntimeChunkResult>(FRuntimeChunkResult{ EDownloadToMemoryResult::DownloadFailed, TArray64<uint8>() }).GetFuture();
	}
	...
	HttpRequestRef->OnProcessRequestComplete().BindLambda([WeakThisPtr, PromisePtr, URL, ChunkRange](FHttpRequestPtr Request, FHttpResponsePtr Response, bool bSuccess) mutable
		{
			...
			PromisePtr->SetValue(FRuntimeChunkResult{ EDownloadToMemoryResult::Success, TArray64<uint8>(Response->GetContent()) });
		});
...
TFuture<FRuntimeChunkResult> FRuntimeChunk::DownloadFileByPayload(const FString& URL, float Timeout, const FString& ContentType, const TFunction<void(int64, int64)>& OnProgress)
{
	if (bCanceled)
	{
		UE_LOG(LogTemp, Warning, TEXT("Canceled file download from %s"), *URL);
	...
	HttpRequestRef->OnProcessRequestComplete().BindLambda([WeakThisPtr, PromisePtr, URL](FHttpRequestPtr Request, FHttpResponsePtr Response, bool bSuccess) mutable
		{
			...
			if (!SharedThis.IsValid())
			{
				UE_LOG(LogTemp, Warning, TEXT("Failed to download file from %s by payload:  has been destroyed"), *URL);
				PromisePtr->SetValue(FRuntimeChunkResult{ EDownloadToMemoryResult::DownloadFailed, TArray64<uint8>() });
				return;
			}
			...
			return PromisePtr->SetValue(FRuntimeChunkResult{ EDownloadToMemoryResult::SucceededByPayload, TArray64<uint8>(Response->GetContent()) });
		});
...
TFuture<int64> FRuntimeChunk::GetContentSize(const FString& URL, float Timeout)
{
...
void FRuntimeChunk::CancelDownload()
{
	bCanceled = true;
