 ...
 */
using FRuntimeChunkResult = struct { EDownloadToMemoryResult Result; TArray64<uint8> Data; };
 ...
 */
class UTILITYFNSLIB_API FRuntimeChunk : public TSharedFromThis<FRuntimeChunk>
{
public:
	FRuntimeChunk();
	virtual ~FRuntimeChunk();
	 ...
	 */
	virtual TFuture<FRuntimeChunkResult> DownloadFile(const FString& URL, float Timeout, const FString& ContentType, int64 MaxChunkSize, const TFunction<void(int64, int64)>& OnProgress);
	 ...
	 * @return A future that resolves to the downloaded data as a TArray64<uint8>
	 */
	virtual TFuture<FRuntimeChunkResult> DownloadFileByChunk(const FString& URL, float Timeout, const FString& ContentType, int64 ContentSize, FInt64Vector2 ChunkRange, const TFunction<void(int64, int64)>& OnProgress);
	 ...
	 */
	virtual TFuture<FRuntimeChunkResult> DownloadFileByPayload(const FString& URL, float Timeout, const FString& ContentType, const TFunction<void(int64, int64)>& OnProgress);
